...
export class ToolExecutor{
  #overlay = new Map()
  #deleted = new Set()
  #norm = (rel)=> path.posix.normalize(rel.split(path.sep).join("/")).replace(/^\.\//, "");
  ...
  #resolveSafe (rel){
    ...
    if(relCheck.startsWith('..') ||  path.isAbsolute(relCheck)){
      throw new Error(`Path excapes workspace: ${rel}` )
    }
    return abs
  }
  ...
  #excluded (relPath){
    ...
    readFile(rel){
      ...
      throw new Error(`File not foundL ${rel}`);}
